defmodule Candil.HTTP do
  ...
  def get(url, headers \\ [], opts \\ []) do
    ...
    case Req.get(url, headers: headers, receive_timeout: timeout) do
      ...
      {:error, reason} when is_atom(reason) and reason == :timeout ->
  ...
  defp wrap_error({:error, reason}) when is_atom(reason) and reason == :timeout do
    {:error, Error.timeout()}
  end
  ...
  defp wrap_reason(reason) when is_atom(reason) and reason == :timeout, do: Error.timeout()
